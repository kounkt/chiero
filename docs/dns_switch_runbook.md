...
**⚠️ 保存できたか必ず確認する**（2026-07-15、①を実行したつもりが保存されていなかった）:
```bash
dig +short MX chiero.jp @ns1.xserver.jp    # 権威DNSに直接聞く＝キャッシュを迂回
# → 「0 sv7600.xserver.jp.」が返れば成功
# → 「0 chiero.jp.」のままなら保存されていない。②へ進まないこと
```
一覧画面に戻って **`chiero.jp / MX / sv7600.xserver.jp`** と表示されているかも目で確認する。

### ② A を GitHub Pages へ向ける

> 🚨 **1つのレコードに4つのIPをまとめて入れてはいけない。**
> 「内容」欄は**1レコード＝1つのIP**。4行を貼ると「『内容』の記述に誤りがあります。」と出る。
> （2026-07-15、この手順書の表が誤解を招いて実際に起きた。表は「最終的にこの4行が並ぶ」という完成形であって、1回の入力内容ではない）

**2-1. 既存の1行を書き換える**

対象行: **`chiero.jp` / `A` / `183.90.241.121`** ← ホスト名が `chiero.jp` のもの。`www.` や `*.` ではない

鉛筆 → 内容を `185.199.108.153` **だけ**にする（他の3つは書かない）→ 設定する

**2-2. 残り3本を1つずつ足す**

「**＋DNSレコード設定を追加**」を押して、以下を**1本ずつ・計3回**登録する。

| 回 | ホスト名 | 種別 | 内容 | TTL |
|---|---|---|---|---|
| 1回目 | （空欄） | A | `185.199.109.153` | 3600 |
| 2回目 | （空欄） | A | `185.199.110.153` | 3600 |
| 3回目 | （空欄） | A | `185.199.111.153` | 3600 |

※ホスト名は空欄でよい（＝ chiero.jp 自体を指す）。`chiero.jp` と入力すると `chiero.jp.chiero.jp` になる場合があるので、**空欄が安全**。

**完成形**（この4行が並んでいればOK）:
```
chiero.jp  A  185.199.108.153  3600
